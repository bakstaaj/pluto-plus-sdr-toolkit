...
if not exist "%RELEASE_ROOT%\configs\airband.conf" (
    echo ERROR: config not found:
    echo   %RELEASE_ROOT%\configs\airband.conf
    pause
    exit /b 1
)
...
"%BIN_DIR%\pluto_scan_session.exe" --config "%RELEASE_ROOT%\configs\airband.conf" %*
...
if exist "%SESSION_DIR%\airband_report.html" (
    start "" "%SESSION_DIR%\airband_report.html"
)
