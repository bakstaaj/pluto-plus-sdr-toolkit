...
if not exist "%RELEASE_ROOT%\configs\fm.conf" (
    echo ERROR: config not found:
    echo   %RELEASE_ROOT%\configs\fm.conf
    pause
    exit /b 1
)
...
"%BIN_DIR%\pluto_scan_session.exe" --config "%RELEASE_ROOT%\configs\fm.conf" %*

if errorlevel 1 (
    echo.
    echo ERROR: FM broadcast scan session failed.
    pause
    exit /b 1
)

if exist "%SESSION_DIR%\fm_report.html" (
    start "" "%SESSION_DIR%\fm_report.html"
)
